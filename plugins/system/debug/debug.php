...
class plgSystemDebug extends JPlugin
{
	...
	public function __construct(&$subject, $config)
	{
		...
		{
			JLog::addLogger(array('text_file' => 'deprecated.log', 'text_file_no_php'=>true), JLog::ALL, array('deprecated'));
		}
	...
	public function __destruct()
	{
		...
		ob_end_clean();
		$_SERVER['HTTP_USER_AGENT'] = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'not detected';
	...
	protected function display($item, array $errors = array())
	{
		...
		$html .= '<div class="' . $class . '"><h3>' . $title . '</h3></div>';


		$html .= '<div class="dbgContainer" id="dbgContainer' . $item . '">';
		$html .= $this->$fncName();
		$html .= '</div>';
	...
	protected function displayQueries()
	{
		...
		foreach ($log as $k => $sql)
		{
			...
			$time = isset($log_times[$k]) ? $log_times[$k] : 0;
			$summary_time += $time;
			$round_time = round($time, 6);
			$class = $round_time >= 0.1 ? 'red' : 'black';

			$html .= '<li><code>' . $text . ' </code><div class="dbgQueryTime"><span class="'.$class.'">'.$round_time.'</span> sec</div></li>';
		}

		$html .= '</ol>';
		$class = $summary_time >= 0.1 ? 'red' : 'black';
		$html .= '<div class="dbgQueryTime">'.JText::_('PLG_DEBUG_QUERY_TIMES_SUMMARY').': <span class="'.$class.'">'.round($summary_time, 6).'</span> sec</div><br />- - -<br />';
	...
	protected function displayLanguageFilesLoaded()
	{
		...
		$loadTimes = JFactory::getLanguage()->getLoadTimes();
		...
		foreach (JFactory::getLanguage()->getPaths() as $extension => $files)
		{
			foreach ($files as $file => $status)
			{
				...
				$time = isset($loadTimes[$extension][$file]) ? $loadTimes[$extension][$file] : 0;
				$summary_time += $time;
				$round_time = round($time, 6);
				$class = $round_time >= 0.1 ? 'red' : 'black';

				$html .= ' <div class="dbgLangLoadTime">(<span class="'.$class.'">'.$round_time.'</span> sec)</div>';
		...
		$class = $summary_time >= 0.1 ? 'red' : 'black';
		$html .= '<div class="dbgLangLoadTime">'.JText::_('PLG_DEBUG_QUERY_TIMES_SUMMARY').' <span class="'.$class.'">'.round($summary_time, 6).'</span> sec</div><br />- - -<br />';
	...
	{
		$newlineKeywords = '#\b(FROM|LEFT|INNER|OUTER|WHERE|SET|VALUES|ORDER|GROUP|HAVING|LIMIT|CASE)\b#i';
	...
	/**
	 * Replaces the LongCMS root with "JROOT" to improve readability.
