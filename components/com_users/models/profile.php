...
class UsersModelProfile extends JModelForm
{
	...
	public function getForm($data = array(), $loadData = true)
	{
		// Get the form.
		$user = PDeals::getUser();
		$form_name = $user->hasProfile() ? 'profile' : 'profile_full';
		$form = $this->loadForm('com_users.profile', $form_name, array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function save($data)
	{
		...
		if (!JComponentHelper::getParams('com_users')->get('change_pers_number') && $user->hasProfile()) {
			unset($data['persNumber']);
		}

		if (!JComponentHelper::getParams('com_users')->get('change_mail')) {
			unset($data['email']);
		}
