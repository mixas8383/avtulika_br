...
class UsersControllerRegistration extends UsersController
{
	...
	public function register()
	{
		...
		if (JDEBUG) {
			if (!$model->checkMail($data)) {
				$app->enqueueMessage(JText::_('COM_USERS_REGISTRATION_DUBLICATE_MAIL'), 'error');
				// Save the data in the session.
				$app->setUserState('com_users.registration.data', $requestData);
				$this->setRedirect(JRoute::_('index.php?option=com_users&view=registration', false));
				return false;
		...
		if ($return === 'adminactivate'){
			$Itemid = JMenu::getItemid('com_users', 'registration', 'complete');
			$this->setMessage(JText::_('COM_USERS_REGISTRATION_COMPLETE_VERIFY'));
			$this->setRedirect(JRoute::_('index.php?option=com_users&view=registration&layout=complete&Itemid='.$Itemid, false));
		} elseif ($return === 'useractivate') {
			$Itemid = JMenu::getItemid('com_users', 'registration', 'complete');
			$this->setMessage(JText::_('COM_USERS_REGISTRATION_COMPLETE_ACTIVATE'));
			$this->setRedirect(JRoute::_('index.php?option=com_users&view=registration&layout=complete&Itemid='.$Itemid, false));
		} else {
			$Itemid = JMenu::getItemid('com_users', 'login');
			$this->setMessage(JText::_('COM_USERS_REGISTRATION_SAVE_SUCCESS'));
			$this->setRedirect(JRoute::_('index.php?option=com_users&view=login&Itemid='.$Itemid, false));
		}
