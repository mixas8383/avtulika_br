...
class AdminModelSysInfo extends JModelLegacy
{
	...
	public function getDirectory() {
		if (is_null($this->directories))
		{
			...
			$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/components', JPATH_ADMINISTRATOR.'/components');
			$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/language', JPATH_ADMINISTRATOR.'/language');

			// List all admin languages
			$admin_langs = JFolder::folders(JPATH_ADMINISTRATOR.'/language');
			foreach($admin_langs as $alang) {
				$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/language/' . $alang, JPATH_ADMINISTRATOR.'/language/'.$alang);
			}

			// List all manifests folders
			$manifests = JFolder::folders(JPATH_ADMINISTRATOR.'/manifests');
			foreach($manifests as $_manifest) {
				$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/manifests/' . $_manifest, JPATH_ADMINISTRATOR.'/manifests/'.$_manifest);
			}

			$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/modules', JPATH_ADMINISTRATOR.'/modules');
			$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/templates', JPATH_THEMES);
			...
			$this->_addDirectory(JFOLDER_ADMINISTRATOR.'/cache', JPATH_CACHE, 'COM_ADMIN_CACHE_DIRECTORY');
