...
{
	private static $_edit_types = array(
			1=>'Proportional',
			2=>'Square',
			3=>'Exactly (Crop From Top)',
			4=>'Exactly (Crop Center)',
		);

	private static $_codenames = array(
			'original',
			'image1',
			'image2',
			'image3',
	...
	private static $_qualities = array(
			10=>10,
			20=>20,
			30=>30,
			40=>40,
	...
	public static function getFreeCodeNames($id = null)
	{
		$db = JFactory::getDBO();
		...
		$query->group('codename');
		$query->order('codename');
		$db->setQuery($query);
		...
		$return = array();
		foreach(self::$_codenames as $arr)
		{
			if (in_array($arr, $items))
			{
				continue;
			}
			$return[] = $arr;
	...
	public static function getEditTypes()
	{
		return self::$_edit_types;
	}
