...
abstract class JLoader
{
	...
	public static function import($key, $base = null)
	{
		...
		if (!isset(self::$imported[$key]))
		{
			...
			// If we are importing a library from the LongCMS namespace set the class to autoload.
			if (strpos($path, 'core') === 0)
			{
				// Since we are in the LongCMS namespace prepend the classname with J.
				$class = 'J' . $class;
			...
			/*
			 * If we are not importing a library from the LongCMS namespace directly include the
	...
	/**
	 * Method to setup the autoloaders for the LongCMS Platform.  Since the SPL autoloaders are
	...
	{
		// Register the base path for LongCMS platform libraries.
		self::registerPrefix('J', JPATH_PLATFORM . '/core');
		self::registerPrefix('P', JPATH_PLATFORM . '/project');
 ...
 * @since   11.1
 */
function p($var, $die = false, $only_return = false)
{
	if (!JDEBUG) {
		return false;
	}
