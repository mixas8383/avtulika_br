...
jimport('core.log.logger');
jimport('core.filesystem.folder');

/**
 * LongCMS Formatted Text File Log class
 *
...
class JLoggerFormattedText extends JLogger
{
	...
	public function __construct(array &$options)
	{
		...
		{
			$this->options['text_file'] = 'error.log';
		}
	...
	public function addEntry(JLogEntry $entry)
	{
		...
		$e = new Exception();
		$trace = $e->getTrace();
		$caller = isset($trace[4]) ? $trace[4] : false;
		if (isset($caller['class']) && isset($caller['function'])) {
			$line .= ' In: '.$caller['class'].'::'.$caller['function'];
		}
