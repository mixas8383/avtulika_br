 ...
 * for the LongCMS Platform to access and manipulate a URI.  Second it obtains the URI of
...
class JURI extends JObject
{
	...
	public static function base($pathonly = false)
	{
		...
		if (empty(self::$base))
		{
			...
			if (trim($live_site) != '')
			{
				...
				{
					self::$base['path'] .= '/'.JFOLDER_ADMINISTRATOR;
				}
			}
			else
			{
	 ...
	 * @since   11.1
	 */
	public static function root($pathonly = false, $path = null, $remove_trailing_slash = false)
	{
		// Get the scheme
		if (empty(self::$root))
		{
		...
		if ($pathonly === false) {
			$url = $remove_trailing_slash ? self::$root['prefix'] . self::$root['path'] : self::$root['prefix'] . self::$root['path'] . '/';
		} else {
			$url = self::$root['path'];
		}
