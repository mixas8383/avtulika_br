...
abstract class JHtmlBehavior
{
	...
	public static function tooltip($selector = '.hasTip', $params = array())
	{
		...
		if (!isset($opt['offset']))
		{
			// Supporting offsets parameter which was working in mootools 1.2 (LongCMS 1.5)
	...
	public static function keepalive()
	{
		...
		if (IS_SITE) {
			$script .= '$(function()';
			$script .= '{ setTimeout(keepAlive, ' . $refreshTime . '); }';
			$script .= ');';
		} else {
			// Include MooTools framework
			self::framework();
	...
	public static function noframes($location = 'top.location.href')
	{
		...
		if (IS_SITE) {
			$js = "$(function() {if (top == self) {document.documentElement.style.display = 'block'; }" .
