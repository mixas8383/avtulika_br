...
class JLanguage extends JObject
{
	...
	/**
	 * A nested array of language files load times that have been loaded
	 * @var    array
	 * @since  11.1
	 */
	protected $loadTimes = array();
	 ...
	 * @since   11.1
	 */
	public function transliterate($string, $from = false, $to = false, $lower = true)
	{
		...
		if ($from && $to)
		{
			$string = JLanguageTransliterate::convert($string, $from, $to);
		}
		if ($lower)
		{
	...
	protected function loadLanguage($filename, $extension = 'unknown', $overwrite = true)
	{
		...
		{
			$strings = $this->parse($filename, $extension);
		}
	 ...
	 * @since   11.1
	 */
	protected function parse($filename, $extension = 'unknown')
	{
		$version = phpversion();
		...
		if (JDEBUG)
		{
			...
			if (!isset($this->loadTimes[$extension]))
			{
				$this->loadTimes[$extension] = array();
			}
			$this->loadTimes[$extension][$filename] = $time;
		}
	...
	/**
	 * Get a list of language files load times that have been loaded.
	 *
	 * @param   string  $extension  An optional extension name.
	 *
	 * @return  array
	 ...
	 * @since   11.1
	 */
	public function getLoadTimes($extension = null)
	{
		if (isset($extension))
		{
			if (isset($this->loadTimes[$extension]))
			{
				return $this->loadTimes[$extension];
			}
	 ...
	 * @since   11.1
	 */
	public function getTag($underscore = false)
	{
		return $underscore ? str_replace('-', '_', $this->metadata['tag']) : $this->metadata['tag'];
	}
	...
	{
		$ex = explode('-', $this->metadata['tag']);
		return !empty($ex[0]) ? $ex[0] : false;
	}
