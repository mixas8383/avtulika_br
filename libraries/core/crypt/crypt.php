...
/**
 * JCrypt is a LongCMS Platform class for handling basic encryption/decryption of data.
 *
 * @package     	LongCMS.Platform
 * @subpackage  Crypt
 * @since       12.1
...
class JCrypt
{
	...
	public static function encryptString($string)
	{
		...
		$encryption_key = $app->getCfg('secret');
	...
	public static function decryptString($string)
	{
		$app = JFactory::getApplication();
		$encryption_key = $app->getCfg('secret');
		$string = str_replace(array(' ', '%3D'), array('+', '='), $string);
		$decrypted = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($encryption_key), base64_decode($string), MCRYPT_MODE_CBC, md5(md5($encryption_key))), "\0");
		return $decrypted;
	}
