...
abstract class JFactory
{
	...
	public static function getUser($id = null, $field = 'id')
	{
		if (is_null($id)) {
			...
			if (property_exists($current, $field) && strval($current->$field) !== strval($id)) {
				$instance = JUser::getInstance($id, $field);
			} else {
				$instance = self::getSession()->get('user');
			}
		}
	...
	public static function getXMLParser($type = '', $options = array())
	{
		...
		switch (strtolower($type))
		{
			...
			case 'simple':
				...
				jimport('core.utilities.simplexml');
				$doc = new JSimpleXML;
				break;
	...
	{
		jimport('core.utilities.xmlelement');
	...
	/**
	 * Return a reference to the {@link JMultipic} object
	 *
	 *
	 * @return  JMultipic object
	 *
	 * @see     JMultipic
	 * @since   11.1
	 */
	public static function getMultipic()
	{
		//get the multipic object
		if (is_null(self::$multipic))
		{
			jimport('core.image.multipic');
			self::$multipic = new JMultiPic();
		}

		return self::$multipic;
	}
	...
	protected static function createDocument()
	{
		...
		// Keep backwards compatibility with LongCMS 1.0
	...
	{
		jimport('core.filesystem.stream');

		// Setup the context; LongCMS UA and overwrite
		$context = array();
	 ...
	 * @since   11.1
	 */
	public static function getBrowser($userAgent = null, $accept = null)
	{
		jimport('core.environment.browser');

		return JBrowser::getInstance($userAgent, $accept);
	}
