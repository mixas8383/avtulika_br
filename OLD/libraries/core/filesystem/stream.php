...
jimport('core.filesystem.helper');
jimport('core.utilities.utility');

/**
 * LongCMS Stream Interface
 *
 * The LongCMS stream interface is designed to handle files as streams
...
class JStream extends JObject
{
		...
		$use_prefix = false, $relative = false, $detectprocessingmode = false)
	{
		...
		if (isset($url['scheme']))
		{
			// If we're dealing with a LongCMS stream, load it
