...
class JPath
{
	...
	public static function replaceAdmin($data)
	{
		if (empty($data))
		{
			return $data;
		...
		if (is_array($data))
		{
			array_walk($data, array('JPath', 'replaceAdminCallback'));
		} else {
			self::replaceAdminCallback((string)$data);
		}
		return $data;
	}

	public static function replaceAdminCallback(&$item)
	{
		$item = str_replace(array('administrator', 'JFOLDER_ADMINISTRATOR'), JFOLDER_ADMINISTRATOR, $item);
	}
}
