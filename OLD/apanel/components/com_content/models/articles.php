...
class ContentModelArticles extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select('CONCAT(ua.name, " ", ua.surname) AS author_name');
		...
		if (!empty($search)) {
			...
			elseif (stripos($search, 'author:') === 0) {
				...
				$query->where('(ua.name LIKE '.$search.' OR ua.surname LIKE '.$search.' OR ua.username LIKE '.$search.')');
			}
			else {
	...
	public function getAuthors() {
		...
		// Construct the query
		$query->select('u.id AS value, CONCAT(u.name, " ", u.surname) AS text');
		$query->from('#__users AS u');
		$query->join('INNER', '#__content AS c ON c.created_by = u.id');
		$query->group('u.id, text');
		$query->order('text');
