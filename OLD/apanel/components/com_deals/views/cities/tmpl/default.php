...
<form action="<?php echo JRoute::_('index.php?option='.JCOMPONENT.'&view=cities'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist">
		...
		foreach ($this->items as $i => $item) :
			$canEdit	= $user->authorise('core.edit', JCOMPONENT.'.city.'.$item->id);
			$canCheckin	= $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
			$canChange	= $user->authorise('core.edit.state', JCOMPONENT.'.city.'.$item->id) && $canCheckin;
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					...
					<?php endif; ?>
					<?php if ($canEdit) : ?>
						<a href="<?php echo JRoute::_('index.php?option='.JCOMPONENT.'&task=city.edit&id='.(int) $item->id); ?>">
