 ...
 * @since       1.6
 */
class DealsModelUser extends JModelAdmin
{
	/**
	...
	*/
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_deals.user', 'user', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_deals.edit.user.data', array());
		...
		if (empty($data))
		{
			...
			if (!empty($data->id)) {
				...
				$query = 'SELECT `balance` '
							.' FROM `#__users_balance` '
							.' WHERE `user_id`="'.$data->id.'" '
							.' LIMIT 1 '
							;
				$db->setQuery($query);
				$balance = $db->loadResult();
				$data->balance = Balance::convertAsMajor($balance);
			}
		}
		...
		// Trigger the data preparation event.
		$results = $dispatcher->trigger('onContentPrepareData', array('com_deals.profile', $data));
	...
	public function save($data)
	{
		...
		if (!empty($data['balance'])) {
			$balance = Balance::convertAsMinor($data['balance']);
	...
	public function updateBalance($user, $balance)
	{
		...
		if ($exists) {
			...
			$query = 'INSERT INTO `#__users_balance` '
						.' SET `balance`="'.(int)$balance.'", `user_id`="'.$user->id.'" '
						;
			$db->setQuery($query);
			$status = $db->query();
		}
	...
	function subscribe(&$pks, $value = 1)
	{
		// Initialise variables.
		$app		= JFactory::getApplication();
		$dispatcher	= JDispatcher::getInstance();
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk))
			{
				$old	= $table->getProperties();
				// Skip changing of same state
				if ($table->mail_subscribed == $value)
				{
					unset($pks[$i]);
					continue;
				}

				$table->mail_subscribed = (int) $value;
	...
	public function getGroups()
	{
		...
		{
			$model = JModelLegacy::getInstance('Groups', 'DealsModel', array('ignore_request' => true));
