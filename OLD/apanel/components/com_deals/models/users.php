 ...
 * @since       1.6
 */
class DealsModelUsers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'balance', 'ab.balance',
				'phone', 'a.phone',
				'persNumber', 'a.persNumber',
				'subscribed', 'a.mail_subscribed',
			);
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$persnum = $this->getUserStateFromRequest($this->context.'.filter.persnum', 'filter_persnum');
		$this->setState('filter.persnum', $persnum);
	...
	public function getUsers($opts = array())
	{
		...
		$state = $jinput->post->get('filter_state');
		if (is_numeric($state))
		{
		...
		$active = $jinput->post->get('filter_active');
		...
		$groupId = $jinput->post->get('filter_group_id');
		$groups = $jinput->post->get('filter_groups');
		...
		if ($jinput->post->get('filter_search') !== '')
		{
			// Escape the search token.
			$token	= $db->Quote('%'.$db->escape($jinput->post->get('filter_search')).'%');

			// Compile the different search clauses.
			$searches	= array();
			$searches[]	= 'a.name LIKE '.$token;
			$searches[]	= 'a.surname LIKE '.$token;
		...
		$persnum = $jinput->post->get('filter_persnum');
		if (!empty($persnum))
		{
			$query->where('a.persNumber = '.$db->escape($persnum));
		}

		$datefrom = $jinput->post->get('filter_datefrom');
		if (!empty($datefrom)) {
			$datefrom .= ' 00:00:00';
			$jdate = JFactory::getDate($datefrom, 'Asia/Tbilisi');
			$date = $jdate->toSql();
			$query->where('a.registerDate >= "'.$date.'"');
		}

		$datetill = $jinput->post->get('filter_datetill');
		if (!empty($datetill)) {
			$datetill .= ' 23:59:59';
		...
		if ($range = $jinput->post->get('filter_range'))
		{
			jimport('core.utilities.date');
		...
		$excluded = $jinput->post->get('filter_excluded');
		if (!empty($excluded))
		{
		...
		$query->select('ab.balance');
		$query->join('LEFT', '#__users_balance AS ab ON a.id = ab.user_id');
	...
	protected function getListQuery()
	{
		...
		$persnum = $this->getState('filter.persnum');
		if (!empty($persnum))
		{
			$query->where('a.persNumber = '.$db->escape($persnum));
		}
