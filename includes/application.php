		...
		try
		{
			...
			switch($document->getType())
			{
				case 'html':
					...
					$lang_data = isset($languages[$lang_code]) ? $languages[$lang_code] : null;


					// Set metadata
					if (!empty($lang_data->metakey)) {
					...
					if ($this->getCfg('og_tags', 0)) {
						$og_title = !empty($lang_data->og_title) ? $lang_data->og_title : $this->getCfg('sitename', '');
						$og_type = !empty($lang_data->og_type) ? $lang_data->og_type : 'website';
						$og_image = !empty($lang_data->og_image) ? JURI::root().$lang_data->og_image : '';
						$og_url = !empty($lang_data->og_url) ? $lang_data->og_url : JURI::root();
						$og_description = !empty($lang_data->og_description) ? $lang_data->og_description : $params->get('page_description');
						$og_admins = $this->getCfg('fb_og_admins', '');
						$og_appid = $this->getCfg('fb_og_appid', '');

						if ($og_title) {
							$document->setMetaData('og:title', $og_title);
						}
						if ($og_type) {
							$document->setMetaData('og:type', $og_type);
						}
						if ($og_image) {
							$document->setMetaData('og:image', $og_image);
						}
						if ($og_url) {
							$document->setMetaData('og:url', $og_url);
						}
						if ($og_description) {
							$document->setMetaData('og:description', $og_description);
						}
						if ($og_admins) {
							$document->setMetaData('og:admins', $og_admins);
						}
						if ($og_appid) {
							$document->setMetaData('og:app_id', $og_appid);
						}
			...
			{
				$document->setGenerator('LongCMS - Universal Content Management System - Version ' . JVERSION);
			}
			else
			{
				$document->setGenerator('LongCMS - Universal Content Management System');
			}
	...
	public function render()
	{
		...
		switch ($format)
		{
			...
			default:
				...
				$offline = $this->getCfg('offline');
				if ($offline && !JDEBUG) {
					$deny = false;
					switch($offline) {
						case 3: // disable for all
							$deny = true;
							break;
						case 2:
							...
							break;
						case 1:
							if (!$user->authorise('core.login.offline') && !$browser->isRobot()) {
								$deny = true;
							}
							break;
					}
